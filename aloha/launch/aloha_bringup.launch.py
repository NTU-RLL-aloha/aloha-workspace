...
def launch_setup(context, *args, **kwargs):
    ...
    robot_name_follower_left_launch_arg = LaunchConfiguration(
        "robot_name_follower_left"
    )
    robot_name_follower_right_launch_arg = LaunchConfiguration(
    ...
    for camera_name in camera_names:
        ...
        rs_actions.append(
            Node(
                ...
                parameters=[tag_config_path_launch_arg],
                remappings=[
                    ("image_rect", f"camera/color/image_rect_raw"),
                    ("camera_info", f"camera/color/camera_info"),
                    ("/tf", "tf"),
                ],
                output="log",
            )
    ...
    joystick_teleop_node = Node(
        ...
        parameters=[
            ParameterFile(
                PathJoinSubstitution(
                    [FindPackageShare("aloha"), "config", "base_joystick_teleop.yaml"]
                ),
                allow_substs=True,
            ),
        ],
    ...
    aloha_collision_launch_include = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [
                PathJoinSubstitution(
                    [FindPackageShare("aloha"), "launch", "aloha_collision.launch.py"]
                ),
            ]
        ),
        launch_arguments={
        ...
        condition=IfCondition(LaunchConfiguration("check_collision")),
    )

    # WARNING: The performance is so fucking bad, please do not use it
    # Add gravity compensation node if enabled
    gravity_compensation_leader_left_node = Node(
            ...
            parameters=[
                {
                    "motor_specs": PathJoinSubstitution([
                        FindPackageShare("aloha"),
                        "config",
                        f"leader_motor_specs_left.yaml",
                    ])
                }
            ],
            output="screen",
    ...
    gravity_compensation_leader_right_node = Node(
            package="interbotix_gravity_compensation",
            executable="interbotix_gravity_compensation",
            name="gravity_compensation",
            ...
            parameters=[
                {
                    "motor_specs": PathJoinSubstitution([
                        FindPackageShare("aloha"),
                        "config",
                        f"leader_motor_specs_right.yaml",
                    ])
                }
            ],
            output="screen",
    ...
    return [
        ...
        joy_node,
        rviz2_node,
        aloha_collision_launch_include,
        gravity_compensation_leader_left_node,
        gravity_compensation_leader_right_node,
    ]
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            default_value="false",
            choices=("true", "false"),
            description="if `true`, launches the collision detection node; if `false`, does not launch it.",
        )
    )
    declared_arguments.append(
        DeclareLaunchArgument(
