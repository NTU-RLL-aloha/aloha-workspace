...
from robomimic.algo.diffusion_policy import ConditionalUnet1D, replace_bn_with_gn
...
class DiffusionPolicy(nn.Module):
    def __init__(self, args_override):
        ...
        self.img_shape = args_override.get("img_shape", None)
        backbones = []
        pools = []
        linears = []
        for _ in self.camera_names:
            ...
            pool_input_shape = [512, 15, 20]
            if self.img_shape is not None:
                pool_input_shape = backbones[-1].output_shape(self.img_shape)
            pools.append(
                SpatialSoftmax(
                    **{
        ...
        ENABLE_EMA = True
        # if ENABLE_EMA:
        #     ema = EMAModel(nets.parameters(), power=self.ema_power)
        # else:
        if ENABLE_EMA:
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None):
        ...
        if actions is not None:  # training time
            ...
            for cam_id in range(len(self.camera_names)):
                ...
                # print("pool_features shape", pool_features.shape)
            ...
            # if self.ema is not None:
            #     self.ema.restore(nets.parameters())
            return naction
    ...
    def deserialize(self, model_dict):
        ...
        if model_dict.get("ema", None) is not None:
            # print("Loaded EMA")
            # print(model_dict["ema"].keys())
            # params = model_dict["ema"]["shadow_params"]
            # ema: nn.Module
            # ema.load_state_dict()
            ...
            import copy
            self.ema.averaged_model = copy.deepcopy(self.nets).eval()
